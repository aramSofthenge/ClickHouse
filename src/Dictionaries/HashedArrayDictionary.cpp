    ...
    if constexpr (dictionary_key_type == DictionaryKeyType::Simple)
    {
        ...
        auto get_parent_func = [&, this](auto & hierarchy_key)
        {
            ...
            if (unlikely(hierarchical_attribute.is_index_null) && (*hierarchical_attribute.is_index_null)[key_index])
                return result;
            const AttributeContainerType<UInt64> & parent_keys_container = std::get<AttributeContainerShardsType<UInt64>>(hierarchical_attribute.containers)[shard];
...
DictionaryHierarchicalParentToChildIndexPtr HashedArrayDictionary<dictionary_key_type, sharded>::getHierarchicalIndex() const
{
    if constexpr (dictionary_key_type == DictionaryKeyType::Simple)
    {
        ...
        index_to_key.reserve(total_element_count);

        for (const auto & key_attribute_container : key_attribute.containers)
        ...
        const auto & hierarchical_attribute_containers = std::get<AttributeContainerShardsType<UInt64>>(hierarchical_attribute.containers);
        for (const auto & parent_keys_container : hierarchical_attribute_containers)
        {
...
void HashedArrayDictionary<dictionary_key_type, sharded>::createAttributes()
{
    ...
    for (const auto & dictionary_attribute : dict_struct.attributes)
    {
        auto type_call = [&, this](const auto & dictionary_attribute_type)
        {
            ...
            auto is_index_null = dictionary_attribute.is_nullable ? std::make_optional<std::vector<bool>>() : std::nullopt;
    ...
    DefaultValueExtractor & default_value_extractor) const
{
    ...
    for (size_t key_index = 0; key_index < keys_size; ++key_index)
    {
        ...
        if (element_index != -1)
        {
            ...
            if constexpr (is_nullable)
                set_value(key_index, element, (*attribute.is_index_null)[found_element_index]);
            else
                set_value(key_index, element, false);
        }
        else
