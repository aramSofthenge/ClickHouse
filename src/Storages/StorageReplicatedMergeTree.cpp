...
std::vector<String> StorageReplicatedMergeTree::getZookeeperZeroCopyLockPaths() const
{
    ...
    std::set<DataSourceType> disk_types_with_zero_copy;
    for (const auto & disk : disks)
    {
        if (!disk->supportZeroCopyReplication())
            continue;

        disk_types_with_zero_copy.insert(disk->getDataSourceDescription().type);
    }
...
void StorageReplicatedMergeTree::drop()
{
    ...
    if (maybe_has_metadata_in_zookeeper)
    {
        ...
        bool last_replica_dropped = dropReplica(zookeeper, zookeeper_path, replica_name, log, getSettings(), &has_metadata_in_zookeeper);
        if (last_replica_dropped)
        {
            dropZookeeperZeroCopyLockPaths(zookeeper, zero_copy_locks_paths, log);
        }
    }
}
...
bool StorageReplicatedMergeTree::dropReplica(const String & drop_zookeeper_path, const String & drop_replica, Poco::Logger * logger)
{
